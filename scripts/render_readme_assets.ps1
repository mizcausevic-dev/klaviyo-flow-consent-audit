...
function New-ProofImage {
    ...
    $graphics.DrawString("KLAVIYO FLOW CONSENT AUDIT", $eyebrowFont, $accentBrush, 92, 92)
...
New-ProofImage -Path (Join-Path $screenshots "01-overview-proof.png") `
    -Title "Consent-safe lifecycle flow posture" `
    -Subtitle "Operator-readable proof for consent evidence, suppression hygiene, deliverability readiness, and send-safe Growth Ops sequencing." `
    -Bullets @(
        "Surface the same packets lifecycle marketing, RevOps, and legal teams argue over.",
        "Keep send debt visible before it becomes a deliverability or compliance problem.",
        "Show recruiter-readable Klaviyo, Growth Ops, and enterprise governance depth."
    )

New-ProofImage -Path (Join-Path $screenshots "02-flow-lane-proof.png") `
    -Title "Lane ownership and flow routing" `
    -Subtitle "Each lane maps to a real Growth Ops operator responsibility instead of generic marketing copy." `
    -Bullets @(
        "Consent, audience, delivery, and measurement lanes stay separated and accountable.",
        "Owner mapping keeps lifecycle marketing, RevOps, and growth engineering explicit.",
        "Findings roll up into send-safe next actions."
    )

New-ProofImage -Path (Join-Path $screenshots "03-delivery-posture-proof.png") `
    -Title "Delivery posture before launch" `
    -Subtitle "Decision packets show which flows can move and which should stop." `
    -Bullets @(
        "Completeness score, blocker, and send window stay in one view.",
        "High-severity packet debt is visible before lifecycle flows go live.",
