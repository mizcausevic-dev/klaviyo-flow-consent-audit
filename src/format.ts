...
export function toSummary(report: CoverageReport) {
  ...
  return `${posture} ${report.programs} flows · ${report.packets} packets · ${report.highSeverityPackets} high-severity packets · ${report.workflowGaps} workflow gaps`;
}
...
export function toMarkdown(report: CoverageReport) {
  ...
  return [
    `# Flow consent posture ${report.ok ? "✅" : "❌"}`,
    "",
    `- Flows: **${report.programs}**`,
    `- On-track flows: **${report.onTrackPrograms}**`,
